...
fnc_metad_group_mcmc <- function(df, str_label, n_adapt = 10000) {
...
fnc_metad_group <-
  function(df,
    ...
    all_samples <- combine.mcmc(mcmc_chains) %>%
      as_tibble
    ...
    # Extract aggregate Mratio results
...
metad_group_all_participants_mcmc <-
  fnc_metad_group_mcmc(counts_all_participants, "all participants")
...
fnc_metad_indiv <- function(df) {
  ...
  # function that takes the list object with the three chains
  # and extracts the posterior information
  fnc_extract_posterior_infos <- function(chains) {
    try({
      # turn mcmc.list into a tidybayes data frame
      ...
      # turn tbl_mcmc back into an mcmc.list to be able to calculate the rhats
      list_mcmc <- tbl_mcmc %>%
        group_by(.chain) %>%
        nest() %>%
      ...
      # by calculating the point estimates and credible intervals
      # plus the rhats
      y <- tbl_mcmc %>%
        ggdist::median_qi() %>%
        bind_cols(rhats)
  ...
  # using `unnest` we add these new variables to the main data frame
...
Exclude meta-d' outliers using the standard box-and-whiskers plot criterion for outliers
applied across all modelled participants (André, 2022; https://doi.org/10.1037/x).

```{r exclude_outliers}
# define function for counting outliers
fnc_outlier_count <- function(df) {
  output <- df %>%
    filter(fnc_outlier(meta_d)) %>%
    nrow()
...
# count outliers
n_outliers_all_participants <-
  fnc_outlier_count(metad_indiv_all_participants)
n_outliers_six_or_more_waves <-
  fnc_outlier_count(metad_indiv_six_or_more_waves)
n_outliers_twelve_waves <-
  fnc_outlier_count(metad_indiv_twelve_waves)
n_outliers_congruent_party_statements <-
  fnc_outlier_count(metad_indiv_congruent_party_statements)
n_outliers_incongruent_party_statements <-
  fnc_outlier_count(metad_indiv_incongruent_party_statements)
n_outliers_neutral_party_statements <-
  fnc_outlier_count(metad_indiv_neutral_party_statements)
n_outliers_congruent_ideology_statements <-
  fnc_outlier_count(metad_indiv_congruent_ideology_statements)
n_outliers_incongruent_ideology_statements <-
  fnc_outlier_count(metad_indiv_incongruent_ideology_statements)
n_outliers_neutral_ideology_statements <-
  fnc_outlier_count(metad_indiv_neutral_ideology_statements)
n_outliers_congruent_truth_party_statements <-
  fnc_outlier_count(metad_indiv_congruent_truth_party_statements)
n_outliers_incongruent_truth_party_statements <-
  fnc_outlier_count(metad_indiv_incongruent_truth_party_statements)
n_outliers_neutral_truth_party_statements <-
  fnc_outlier_count(metad_indiv_neutral_truth_party_statements)
n_outliers_congruent_truth_ideology_statements <-
  fnc_outlier_count(metad_indiv_congruent_truth_ideology_statements)
n_outliers_incongruent_truth_ideology_statements <-
  fnc_outlier_count(metad_indiv_incongruent_truth_ideology_statements)
n_outliers_neutral_truth_ideology_statements <-
  fnc_outlier_count(metad_indiv_neutral_truth_ideology_statements)
n_outliers_recognized_statements <-
  fnc_outlier_count(metad_indiv_recognized_statements)
n_outliers_non_recognized_statements <-
  fnc_outlier_count(metad_indiv_non_recognized_statements)
n_outliers_satire_statements <-
  fnc_outlier_count(metad_indiv_satire_statements)
n_outliers_non_satire_statements <-
  fnc_outlier_count(metad_indiv_non_satire_statements)

# define function for excluding outliers
fnc_outlier_exclusion <- function(df) {
  output <- df %>%
    filter(fnc_outlier(meta_d)) %>%
    select(id) %>%
    anti_join(df, ., by = "id")
...
metad_indiv_all_participants <-
  fnc_outlier_exclusion(metad_indiv_all_participants)
metad_indiv_six_or_more_waves <-
  fnc_outlier_exclusion(metad_indiv_six_or_more_waves)
metad_indiv_twelve_waves <-
  fnc_outlier_exclusion(metad_indiv_twelve_waves)
metad_indiv_congruent_party_statements <-
  fnc_outlier_exclusion(metad_indiv_congruent_party_statements)
metad_indiv_incongruent_party_statements <-
  fnc_outlier_exclusion(metad_indiv_incongruent_party_statements)
metad_indiv_neutral_party_statements <-
  fnc_outlier_exclusion(metad_indiv_neutral_party_statements)
metad_indiv_congruent_ideology_statements <-
  fnc_outlier_exclusion(metad_indiv_congruent_ideology_statements)
metad_indiv_incongruent_ideology_statements <-
  fnc_outlier_exclusion(metad_indiv_incongruent_ideology_statements)
metad_indiv_neutral_ideology_statements <-
  fnc_outlier_exclusion(metad_indiv_neutral_ideology_statements)
metad_indiv_congruent_truth_party_statements <-
  fnc_outlier_exclusion(metad_indiv_congruent_truth_party_statements)
metad_indiv_incongruent_truth_party_statements <-
  fnc_outlier_exclusion(metad_indiv_incongruent_truth_party_statements)
metad_indiv_neutral_truth_party_statements <-
  fnc_outlier_exclusion(metad_indiv_neutral_truth_party_statements)
metad_indiv_congruent_truth_ideology_statements <-
  fnc_outlier_exclusion(metad_indiv_congruent_truth_ideology_statements)
metad_indiv_incongruent_truth_ideology_statements <-
  fnc_outlier_exclusion(metad_indiv_incongruent_truth_ideology_statements)
metad_indiv_neutral_truth_ideology_statements <-
  fnc_outlier_exclusion(metad_indiv_neutral_truth_ideology_statements)
metad_indiv_recognized_statements <-
  fnc_outlier_exclusion(metad_indiv_recognized_statements)
metad_indiv_non_recognized_statements <-
  fnc_outlier_exclusion(metad_indiv_non_recognized_statements)
metad_indiv_satire_statements <-
  fnc_outlier_exclusion(metad_indiv_satire_statements)
metad_indiv_non_satire_statements <-
  fnc_outlier_exclusion(metad_indiv_non_satire_statements)

# create df for meta-d' values (as the other df's will be further manipulated below)
metad_indiv_all_participants_meta_d_cleaned <-
  metad_indiv_all_participants

```

Based on our criterion for detecting outliers, we exclude the following numbers
of participants' individual-level point estimates for meta-d' across different models:
* All participants: `r n_outliers_all_participants` participants
* Six or more waves: `r n_outliers_six_or_more_waves` participants
* Twelve waves: `r n_outliers_twelve_waves` participants
* Congruent statements (party only): `r n_outliers_congruent_party_statements` participants
* Incongruent statements (party only): `r n_outliers_incongruent_party_statements` participants
* Neutral statements (party only): `r n_outliers_neutral_party_statements` participants
* Congruent statements (ideology only): `r n_outliers_congruent_ideology_statements` participants
* Incongruent statements (ideology only): `r n_outliers_incongruent_ideology_statements` participants
* Neutral statements (ideology only): `r n_outliers_neutral_ideology_statements` participants
* Congruent statements (party*truth): `r n_outliers_congruent_truth_party_statements` participants
* Incongruent statements (party*truth): `r n_outliers_incongruent_truth_party_statements` participants
* Neutral statements (party*truth): `r n_outliers_neutral_truth_party_statements` participants
* Congruent statements (ideology*truth): `r n_outliers_congruent_truth_ideology_statements` participants
* Incongruent statements (ideology*truth): `r n_outliers_incongruent_truth_ideology_statements` participants
* Neutral statements (ideology*truth): `r n_outliers_neutral_truth_ideology_statements` participants
* Recognized statements: `r n_outliers_recognized_statements` participants
* Non-recognized statements: `r n_outliers_non_recognized_statements` participants
* Satire statements: `r n_outliers_satire_statements` participants
* Non-satire statements: `r n_outliers_non_satire_statements` participants
...
```{r exclude_uninterpretable_values}
...
write_csv(
  metad_indiv_all_participants_meta_d_cleaned,
  here(
    "output/models/indiv/metad_cleaned/metad_indiv_all_participants_meta_d_cleaned.csv"
  )
)
