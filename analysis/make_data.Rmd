...
# load panel data, stratify by ideology, and subsample 20% of all participants
set.seed(12345)
subsample <- read_tsv(here("data/panelPublic.tab")) %>%
  ...
  ungroup %>% # to undo `rowwise`
  mutate(ideology_index_rounded = round(ideology_index, digit = 0)) %>%
  group_by(ideology_index_rounded) %>%
  sample_frac(size = .2)

# create exploratory dataset with 20% subsample
raw <- read_tsv(here("data/panelPublic.tab")) %>%
  mutate(id = row_number()) %>%
  filter(id %in% subsample$id)
...
```{r participants}
participants <-
  list(demos, political_views, psychological_factors) %>%
...
n_participants_final <- raw_n_initial - n_participants_excluded
